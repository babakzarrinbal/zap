...
exports.processCommandLineArguments = void 0;
var yargs = require('yargs');
var path = require('path');
var os = require('os');
var fs = require('fs');
var restApi = require('../../src-shared/rest-api.js');
var commonUrl = require('../../src-shared/common-url.js');
var env = require('./env');
function environmentVariablesDescription() {
    var vars = env.environmentVariable;
    var desc = '';
    Object.keys(vars).forEach(function (key) {
        desc = desc.concat("  ".concat(vars[key].name, ": ").concat(vars[key].description, "\n"));
    });
    return desc;
}
/**
...
function processCommandLineArguments(argv) {
    ...
    var ret = y
        ...
        .version("Version: ".concat(zapVersion.version, "\nFeature level: ").concat(zapVersion.featureLevel, "\nHash: ").concat(zapVersion.hash, "\nDate: ").concat(zapVersion.date))
        .help()
        .alias({
        help: ['h', '?'],
    })
        .epilogue("Environment variables:\n".concat(environmentVariablesDescription(), "\nFor more information, see ").concat(commonUrl.projectUrl))
        .wrap(null)
        .parse(argv);
    // Collect files that are passed as loose arguments
    var allFiles = ret._.filter(function (arg, index) {
        if (index == 0)
            return false;
    ...
    if (ret.tempState) {
        var tempDir = fs.mkdtempSync("".concat(os.tmpdir()).concat(path.sep, "zap."));
        console.log("Using temporary state directory: ".concat(tempDir));
