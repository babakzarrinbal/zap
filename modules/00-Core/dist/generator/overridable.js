...
function nonAtomicType(arg) {
    if (arg === void 0) { arg = { name: 'unknown', isStruct: false }; }
    if (arg.isStruct) {
        return arg.name;
    }
    else {
        return "EmberAf".concat(arg.name);
    }
}
/**
...
function atomicType(arg) {
    if (arg === void 0) { arg = { name: 'unknown', size: 0 }; }
    var name = arg.name;
    var size = arg.size;
    if (name.startsWith('int')) {
        ...
        if (name.endsWith('s'))
            ...
            signed = false;
        var ret = "".concat(signed ? '' : 'u', "int").concat(size * 8, "_t");
        // few exceptions
        ret = cleanseUints(ret, size * 8, signed);
        return ret;
    }
    else if (name.startsWith('enum') || name.startsWith('data')) {
        return cleanseUints("uint".concat(name.slice(4), "_t"), name.slice(4), false);
    }
    else if (name.startsWith('bitmap')) {
        return cleanseUints("uint".concat(name.slice(6), "_t"), name.slice(6), false);
    }
    else {
        switch (name) {
            case 'utc_time':
            ...
            case 'boolean':
                return 'uint8_t';
            case 'array':
                return "/* TYPE WARNING: ".concat(name, " array defaults to */ uint8_t * ");
            default:
                return "/* TYPE WARNING: ".concat(name, " defaults to */ uint8_t * ");
        }
    }
}
