...
const dbApi = require('zap/dist/src-electron/db/db-api');
const dbEnum = require('zap/dist/src-shared/db-enum.js');
const env = require('zap/dist/src-electron/util/env');
const zclLoader = require('zap/dist/src-electron/zcl/zcl-loader.js');
const windowJs = require('zap/dist/src-electron/ui/window');
const httpServer = require('zap/dist/src-electron/server/http-server.js');
const ipcServer = require('zap/dist/src-electron/server/ipc-server');
const ipcClient = require('zap/dist/src-electron/client/ipc-client');
const generatorEngine = require('zap/dist/src-electron/generator/generation-engine.js');
const querySession = require('zap/dist/src-electron/db/query-session.js');
const util = require('zap/dist/src-electron/util/util.js');
const importJs = require('zap/dist/src-electron/importexport/import.js');
const exportJs = require('zap/dist/src-electron/importexport/export.js');
const uiJs = require('zap/dist/src-electron/ui/ui-util');
...
async function startNormal(argv, options = {
    ...
    let uiEnabled = false;
    let db = await dbApi.initDatabaseAndLoadSchema(env.sqliteFile(), env.schemaFile(), env.zapVersion());
    ...
    return zclLoader
        ...
        .then((ctx) => generatorEngine.loadTemplates(ctx.db, argv.generationTemplate))
        .then((ctx) => {
        if (ctx.error) {
        ...
        if (uiEnabled) {
            ...
            if (zapFiles.length == 0) {
                ...
                return util.executePromisesSequentially(zapFiles, (f) => uiJs.openFileConfiguration(f, port));
            }
        }
        else {
            if (showUrl) {
        ...
        // .then(() => {
    //     if (argv.noServer && app != null && options.quit)
    //         app.quit();
    // })
        .catch((err) => {
        env.logError(err);
        ...
        filesArg.forEach((f) => {
            ...
            if (stat.isDirectory()) {
                ...
                dirents.forEach((element) => {
                    if (element.name.endsWith(options.suffix.toLowerCase()) ||
...
async function startConvert(argv, options = {
    ...
    let db = await dbApi.initDatabaseAndLoadSchema(dbFile, env.schemaFile(), env.zapVersion());
    ...
    return util
        .executePromisesSequentially(files, (singlePath, index) => importJs
        ...
        return util
            ...
            if (argv.postImportScript) {
                return importJs.executePostImportScript(db, importResult.sessionId, argv.postImportScript);
            }
        })
...
async function startAnalyze(argv, options = {
    ...
    return dbApi
        ...
        .then((d) => {
        return util.executePromisesSequentially(paths, (singlePath) => importJs
            .importDataFromFile(db, singlePath, {
            defaultZclMetafile: argv.zclProperties,
            postImportScript: argv.postImportScript,
        })
 ...
 */
async function startSelfCheck(argv, options = {
    quit: true,
    cleanDb: true,
    logger: console.log,
}) {
...
async function generateSingleFile(db, f, templatePackageId, outputPattern, index, options = {
    logger: console.log,
    ...
    let genResult = await generatorEngine.generateAndWriteFiles(db, sessionId, templatePackageId, output, options);
...
async function startGeneration(argv, options = {
    ...
    let hrstart = process.hrtime.bigint();
    options.logger(`🤖 ZAP generation started: 
    ...
    if (options.cleanDb && fs.existsSync(dbFile))
        fs.unlinkSync(dbFile);
    let mainDb = await dbApi.initDatabaseAndLoadSchema(dbFile, env.schemaFile(), env.zapVersion());
    ...
    await util.executePromisesSequentially(files, (f, index) => generateSingleFile(mainDb, f, ctx.packageId, output, index, options));
    // if (options.quit && app != null)
    //     app.quit();
}
/**
...
async function startUpMainInstance(isElectron, argv) {
    ...
    if (argv._.includes('status')) {
        ...
        if (argv.zapFiles.length < 1)
            throw 'You need to specify at least one zap file.';
        if (argv.output == null)
            throw 'You need to specify output file.';
